...
(function (global) {
  ...
  Cat.prototype.pickDescription = function (index) {
    var info = this.info;
    while (index >= 0) {
      if (info && info.desc && info.desc[index] != null) return info.desc[index];
      index -= 1;
    }
    return undefined;
  };

  // mirrors Cat#pick_title
  Cat.prototype.pickTitle = function (index) {
    var pickedName = this.pickName(index);
    var all = (this.info && this.info.name) ? this.info.name.join(' | ') : '';
    var names = all.replace(pickedName, '*' + pickedName);
    var desc = this.pickDescription(index);
    return names + '\n' + (desc == null ? '' : desc);
  };

  // resolved image src for the requested form (endpoint computed it), or null
  Cat.prototype.imgSrc = function () {
    return (this.info && this.info.img) || null;
  };

  // mirrors Cat#score_rarity_label (score-window based; nil score -> :rare)
  Cat.prototype.scoreRarityLabel = function () {
    var s = this.score;
    if (s == null || s < 6470) return 'rare';
    if (s < 6970) return 'supa_fest';
    if (s < 9070) return 'supa';
    if (s < 9470) return 'uber_fest';
    if (s < 9940) return 'uber';
    if (s < 9970) return 'legend_fest';
    return 'legend';
  };

  // mirrors Cat#cat_rarity_label
  Cat.prototype.catRarityLabel = function () {
    if (this.rarity === 3) return 'supa';
    if (this.rarity === 4) return 'uber';
    if (this.rarity === 5) return 'legend';
    return 'rare';
  };

  // Kept for local callers while they migrate to the upstream terminology.
  Cat.prototype.rarityLabel = Cat.prototype.scoreRarityLabel;
  ...
  // Mirrors FindCat#search_from_cats: returns Map id -> first matching cat,
  // preserving insertion order (so the Found list order matches the server).
  function searchFromCats(rows, guaranteed, remainingIds) {
    var result = new Map();
    for (var i = 0; i < rows.length; i++) {
      var ab = rows[i];
      var remaining = remainingIds.filter(function (id) { return !result.has(id); });
      for (var r = 0; r < remaining.length; r++) {
        ...
        for (var c = 0; c < ab.length; c++) {
          ...
          if (id === cat.id) {
            ...
            result.set(id, cat.guaranteed);
          }
        }
      }
      if (result.size === remainingIds.length) break;
    }
    return result;
  }

  // Mirrors FindCat#search_all_from_cats: id -> [all matching cats] (numbers).
  function searchAllFromCats(rows, guaranteed, remainingIds) {
    var result = new Map();
    for (var i = 0; i < rows.length; i++) {
      var ab = rows[i];
      for (var r = 0; r < remainingIds.length; r++) {
        var id = remainingIds[r];
        for (var c = 0; c < ab.length; c++) {
          var cat = ab[c];
          if (id === cat.id) {
            if (!result.has(id)) result.set(id, []);
            result.get(id).push(cat);
          } else if (guaranteed && cat.guaranteed && id === cat.guaranteed.id) {
            if (!result.has(id)) result.set(id, []);
            result.get(id).push(cat.guaranteed);
          }
        }
      }
    }
  ...
  // Mirrors FindCat#search_deep + search_from_rolling. Returns Map id -> cat.
  // The rolling beyond `count` also extends the chain (last row's .next).
  Gacha.prototype.runFindCat = function (cats, ids, guaranteed, max) {
    ...
    while (sequence <= max) {
      ...
      var remaining = ids.filter(function (id) { return !found.has(id); });
      var more = searchFromCats([newAb], guaranteed, remaining);
      more.forEach(function (cat, id) { if (!found.has(id)) found.set(id, cat); });
      sequence += 1;
    }
    return found;
  };

  // Mirrors FindCat#search results: [{ cat, numbers: [..] }, ...] incl. missing.
  function buildFoundResults(found, occurrences, ids, pool, max) {
    var results = [];
    found.forEach(function (cat, id) {
      var occ = occurrences.get(id) || [cat];
      results.push({ cat: cat, numbers: occ.map(function (c) { return c.number(); }) });
    });

    var uniqIds = uniq(ids);
    if (found.size < uniqIds.length) {
      uniqIds.filter(function (id) { return !found.has(id); })
        .forEach(function (id) {
          var cat = new Cat({ id: id, info: pool.digCat(id), sequence: max });
          results.push({ cat: cat, numbers: [cat.number()] });
        });
    }
    return results;
  }
  ...
  function buildTracks(poolJson, seed, opts) {
    ...
    // FindCat::Max), which can link the last visible row forward. We also build
    // the Found list (id -> occurrences) for the found_cats panel.
    var foundCats = [];
    if (opts.findCat !== false) {
      var ids = findTargetIds(pool, opts.find || 0);
      if (ids.length) {
        var guaranteed = opts.guaranteed !== false;
        var occurrences = searchAllFromCats(cats, guaranteed, uniq(ids));
        var max = opts.max || 999;
        var found = gacha.runFindCat(cats, ids, guaranteed, max);
        foundCats = buildFoundResults(found, occurrences, ids, pool, max);
      }
    }

    return { cats: cats, gacha: gacha, pool: pool, foundCats: foundCats };
  }
