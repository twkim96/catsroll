(function (global) {
  ...
  var isIPad = /\biPad\b/.test(userAgent) ||
    (/\bMacintosh\b/.test(userAgent) && navigator.maxTouchPoints > 1);
  ...
  function writeSaved(state) {
    if (global.MultiShareApp && global.MultiShareApp.isActive()) return;
    try {
  ...
  function cellHtml(cat, row) {
    ...
    if (cat.rerolled) {
      parts.push('<span class="multi-cat-guaranteed',
        exclusiveSet[cat.rerolled.id] ? ' multi-inline-exclusive' : '', '">R: ',
    ...
    if (cat.guaranteed) {
      parts.push('<span class="multi-cat-guaranteed',
        exclusiveSet[cat.guaranteed.id] ? ' multi-inline-exclusive' : '', '">G: ',
  ...
  function renderTrackTable(outputs, track, label) {
    ...
    for (var i = 0; i < state.count; i++) {
      html.push('<tr><td id="multi-N', i + 1, label,
        '" data-route-number-position="', i + 1, label, '">', i + 1, label, '</td>',
