(function (global) {
  ...
  var seriesSearchApi = global.EventSeriesSearch;
  if (!root || !global.TrackEngine || !seriesSearchApi) {
    return;
  }
  ...
  var limits = data.limits || { rows: 8, count: 999 };
  ...
  var els = {
    ...
    filterSearchRoot: doc.querySelector("[data-multi-filter-query]"),
    filterSearchChips: doc.querySelector("[data-multi-filter-search-chips]"),
    filterSuggestions: doc.querySelector("[data-multi-filter-suggestions]")
  };
  ...
  var filterSearchControl = seriesSearchApi.createAutocomplete({
    input: els.filterSearch,
    root: els.filterSearchRoot,
    chips: els.filterSearchChips,
    suggestions: els.filterSuggestions,
    onChange: renderFilterModal
  });
  ...
  function loadFilterCatalog(lang) {
    ...
    filterCatalogPromises[lang] = global.fetch(url, { credentials: "same-origin" })
      .then(function (response) {
        ...
        var prepared = seriesSearchApi.prepareCatalog(payload);
        filterCatalogs[lang] = prepared.series.filter(function (item) {
          return !isNaN(item.id) && item.count > 0;
        });
        filterCharacters[lang] = prepared.characters;
  ...
  function renderFilterModal() {
    ...
    reset.disabled = filterDraft.length === 0 && !filterSearchControl.hasCriteria();
    ...
    status.hidden = true;
    var visible = filterSearchControl.filterSeries(catalog);
  ...
  function openFilter(index) {
    ...
    filterSearchControl.reset(true);
    filterSearchControl.setCharacters(filterCharacters[lang] || []);
    var catalogPromise = loadFilterCatalog(lang);
    renderFilterModal();
    els.filterDialog.showModal();
    catalogPromise.then(function () {
      if (filterRowIndex === index && els.filterDialog.open) {
        filterSearchControl.setCharacters(filterCharacters[lang] || []);
        renderFilterModal();
      }
    }).catch(function () {
  ...
  function guaranteedNextLabel(cat) {
    if (!cat || !cat.next) return "";
    return nextLabel(cat) || "-> " + cat.next.number();
  }
  ...
  function guaranteedLineHtml(cat, row, label, variant) {
    if (!cat) return "";
    var parts = ['<span class="multi-cat-guaranteed',
      exclusiveSet[cat.id] ? ' multi-inline-exclusive' : '',
      '" data-plan-pick-kind="guaranteed" data-plan-pick-variant="',
      h(variant), '">', h(label), ': ', catNameHtml(cat, row)];
    var next = guaranteedNextLabel(cat);
    if (next) parts.push(' ', h(next));
    parts.push('</span>');
    return parts.join("");
  }
  ...
  function cellHtml(cat, row) {
    ...
    if (cat.guaranteed) {
      parts.push(guaranteedLineHtml(cat.guaranteed, row, "G", "base"));
    }
    if (cat.rerolled && cat.rerolled.guaranteed) {
      parts.push(guaranteedLineHtml(
        cat.rerolled.guaranteed, row, "RG", "rerolled"));
    }
    return parts.join("");
  }
  ...
  function planPickedCat(mark) {
    if (!lastOutputs) return null;
    ...
    if (!match || !item || !item.out || !item.out.cats) return null;
    var index = parseInt(match[1], 10) - 1;
    var track = match[2] === "B" ? 1 : 0;
    var cat = item.out.cats[index] && item.out.cats[index][track];
    var guaranteed = mark.variant === "rerolled" ?
      cat && cat.rerolled && cat.rerolled.guaranteed : cat && cat.guaranteed;
    return mark.kind === "guaranteed" ? guaranteed :
  ...
  function planSelectedCats(marks) {
    ...
    (Array.isArray(marks) ? marks : []).slice().sort(function (left, right) {
      ...
      var selectedCat = planPickedCat(mark);
      if (!selectedCat || [2, 3, 4, 5].indexOf(selectedCat.rarity) === -1) return;
      selected.push({
        id: selectedCat.id,
  ...
  var trackLayoutWidth = els.tables.clientWidth;
  global.addEventListener("resize", function () {
    // Mobile browser chrome changes viewport height without changing wrapping.
    // Resetting every row in that case causes needless layout and scroll jumps.
    var width = els.tables.clientWidth;
    if (width !== trackLayoutWidth) {
      trackLayoutWidth = width;
      scheduleTrackRowSync();
      scheduleTrackScrollUi();
    }
    scheduleVirtualWindow();
  ...
  global.MultiTrackApp = {
    ...
    getPlanMarkLabel: function (mark) {
      var cat = planPickedCat(mark);
      return cat ? String(cat.pickName(formIndex)) : "";
    },
