(function (root, factory) {
  ...
  function search(rawInput, callbacks) {
    ...
    var best = null;
    var paretoGoals = [];
    var paretoMinCostUnits = null;
    var limitHit = false;

    function paretoAllowance(costUnits) {
    ...
    function consider(nodeId) {
      ...
      if (optimization === "pareto") {
        paretoGoals.push(nodeId);
        if (paretoMinCostUnits == null || node.costUnits < paretoMinCostUnits) {
          paretoMinCostUnits = node.costUnits;
        }
        return;
      }
    ...
    for (var offset = 0; offset <= maxStartOffset; offset += 1) {
      ...
      for (var bucketIndex = 0; bucketIndex < bucket.length; bucketIndex += 1) {
        ...
        if (optimization === "pareto" && paretoMinCostUnits != null &&
            state.costUnits > paretoMinCostUnits +
              paretoAllowance(paretoMinCostUnits)) continue;
    ...
    if (optimization === "pareto" && paretoGoals.length) {
      var aliveGoals = paretoGoals.map(function (nodeId) {
        return nodes[nodeId];
      }).filter(function (node) { return node && node.alive; });
      if (aliveGoals.length) {
        var minCostUnits = aliveGoals.reduce(function (minimum, node) {
          return Math.min(minimum, node.costUnits);
        }, Number.MAX_SAFE_INTEGER);
        var allowanceUnits = paretoAllowance(minCostUnits);
        var budgetUnits = minCostUnits + allowanceUnits;
        var selectedGoal = null;
        aliveGoals.forEach(function (node) {
          if (node.costUnits > budgetUnits) return;
          if (!selectedGoal || node.harvestDraws > selectedGoal.harvestDraws ||
              (node.harvestDraws === selectedGoal.harvestDraws &&
                (node.costUnits < selectedGoal.costUnits ||
                  (node.costUnits === selectedGoal.costUnits &&
                    (node.offset < selectedGoal.offset ||
                      (node.offset === selectedGoal.offset &&
                        node.order < selectedGoal.order)))))) {
            selectedGoal = node;
          }
        });
        if (selectedGoal) {
          best = {
            nodeId: nodes.indexOf(selectedGoal),
            offset: selectedGoal.offset,
            scoreUnits: selectedGoal.costUnits,
            costUnits: selectedGoal.costUnits,
            mask: selectedGoal.mask,
            guaranteedUsed: selectedGoal.guaranteedUsed,
            platinumUsed: selectedGoal.platinumUsed,
            legendTicketUsed: selectedGoal.legendTicketUsed,
            harvestDraws: selectedGoal.harvestDraws,
            paretoBaseCostUnits: minCostUnits,
            paretoAllowanceUnits: allowanceUnits,
            paretoBudgetUnits: budgetUnits,
            order: selectedGoal.order
          };
        }
      }
    }
