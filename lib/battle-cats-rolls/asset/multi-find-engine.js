(function (root, factory) {
  ...
  var PARETO_STATE_LIMIT = 400000;
  var MAX_TARGETS = 30;
  var COST_SCALE = 1000;
  var REGULAR_COST = 20;
  var REGULAR_COST_40 = 27;
  var REGULAR_COST_50 = 33;
  var GUARANTEED_COST = 1000;
  var PLATINUM_TICKET_COST = 1000;
  var LEGEND_TICKET_COST = 2000;
  var UBER_CREDIT = 140;
  var LEGEND_CREDIT = 140;
  ...
  function poolIds(pool) {
    ...
    [RARE, SUPA, UBER, LEGEND].forEach(function (rarity) {
  ...
  function regularCostUnits(optimization) {
    if (optimization === "cost40") return REGULAR_COST_40;
    if (optimization === "cost50") return REGULAR_COST_50;
    return REGULAR_COST;
  }
  ...
  function scheduleDayFor(source, currentDay, enabled) {
    if (!enabled) return currentDay;
    var startDay = source && source.startDay;
    var endDay = source && source.endDay;
    if (startDay == null || endDay == null || endDay <= startDay) return null;
    var nextDay = Math.max(currentDay, startDay);
    return nextDay < endDay ? nextDay : null;
  }
  ...
  function ticketCost(kind) {
    return kind === "legend" ? LEGEND_TICKET_COST : PLATINUM_TICKET_COST;
  }
  ...
  function normalizeTickets(input) {
    ...
    return source.filter(function (ticket) {
      ...
      return {
        ...
        startDay: dateDay(ticket.start_on),
        endDay: dateDay(ticket.end_on),
        pool: ticket.pool
      };
    });
  }
  ...
  function paretoAllowanceUnits(costUnits) {
    return Math.max(200, Math.ceil(costUnits * 0.15));
  }
  ...
  function search(rawInput, callbacks) {
    ...
    var input = rawInput || {};
    if (input.optimization === "pareto" && input._paretoBudgetUnits == null) {
      var costInput = Object.assign({}, input, { optimization: "cost" });
      var costResult = search(costInput, callbacks);
      if (costResult.status !== "success") {
        costResult.elapsedMs = Date.now() - startedAt;
        return costResult;
      }

      var baseCostUnits = Number(costResult.costUnits) || 0;
      var allowanceUnits = paretoAllowanceUnits(baseCostUnits);
      var budgetUnits = baseCostUnits + allowanceUnits;
      var firstPassExplored = Number(costResult.explored) || 0;
      var secondCallbacks = callbacks;
      if (callbacks.progress) {
        secondCallbacks = {
          aborted: callbacks.aborted,
          progress: function (progress) {
            progress = progress || {};
            callbacks.progress({
              explored: firstPassExplored + (Number(progress.explored) || 0),
      ...
      var requestedStateLimit = Math.max(1000,
        parseInt(input.stateLimit, 10) || DEFAULT_STATE_LIMIT);
      var paretoInput = Object.assign({}, input, {
        stateLimit: Math.min(requestedStateLimit, PARETO_STATE_LIMIT),
        _paretoBaseCostUnits: baseCostUnits,
        _paretoAllowanceUnits: allowanceUnits,
        _paretoBudgetUnits: budgetUnits
      });
      var paretoResult = search(paretoInput, secondCallbacks);
      var secondPassExplored = Number(paretoResult.explored) || 0;
      paretoResult.paretoFirstPassExplored = firstPassExplored;
      paretoResult.paretoSecondPassExplored = secondPassExplored;
      paretoResult.explored = firstPassExplored + secondPassExplored;
      paretoResult.elapsedMs = Date.now() - startedAt;
      return paretoResult;
    }
    ...
    var events = (Array.isArray(input.events) ? input.events : []).filter(function (event) {
      ...
      return {
        ...
        label: event.label,
        lang: event.lang,
        startDay: dateDay(event.start_on),
        endDay: dateDay(event.end_on),
        pool: event.pool
      };
    });
    var tickets = normalizeTickets(input);
    var optimization = input.optimization === "distance" ? "distance" :
      (input.optimization === "balance" ? "balance" :
        (input.optimization === "pareto" ? "pareto" :
          (input.optimization === "cost40" ? "cost40" :
            (input.optimization === "cost50" ? "cost50" : "cost"))));
    var regularCost = regularCostUnits(optimization);
    var scheduleAware = input.scheduleAware === true;
    ...
    var targetMask = 0;
    var targetCatIds = Object.create(null);
    targets.forEach(function (target, index) {
      targetMask = (targetMask | targetBit(index)) >>> 0;
      targetCatIds[target.cat_id] = true;
    });

    var targetMinCostUnits = [];
    if (optimization === "pareto") {
      var eventTargetIds = Object.create(null);
      events.forEach(function (event) {
        var ids = poolIds(event.pool);
        Object.keys(ids).forEach(function (id) { eventTargetIds[id] = true; });
      });
      var ticketTargetMinCosts = Object.create(null);
      tickets.forEach(function (ticket) {
        if (maxTickets[ticket.kind] <= 0) return;
        var cost = ticketCost(ticket.kind);
        var ids = poolIds(ticket.pool);
        Object.keys(ids).forEach(function (id) {
          if (ticketTargetMinCosts[id] == null || cost < ticketTargetMinCosts[id]) {
            ticketTargetMinCosts[id] = cost;
          }
        });
      });
      targetMinCostUnits = targets.map(function (target) {
        if (eventTargetIds[target.cat_id]) return REGULAR_COST;
        if (target.allow_ticket && ticketTargetMinCosts[target.cat_id] != null) {
          return ticketTargetMinCosts[target.cat_id];
        }
        return REGULAR_COST;
      });
    }
    ...
    var order = 0;
    var best = null;
    var paretoBaseCostUnits = optimization === "pareto" ?
      Math.max(0, Number(input._paretoBaseCostUnits) || 0) : null;
    var paretoAllowance = optimization === "pareto" ?
      Math.max(0, Number(input._paretoAllowanceUnits) || 0) : null;
    var paretoBudgetUnits = optimization === "pareto" ?
      Math.max(0, Number(input._paretoBudgetUnits) || 0) : null;
    var limitHit = false;
    var paretoPrunedTargetCost = 0;
    var paretoPrunedTargetCapacity = 0;
    var paretoPrunedHarvestBound = 0;

    function harvestCount(pulls) {
      return (pulls || []).reduce(function (count, pull) {
        return count + ((pull.rarity === UBER || pull.rarity === LEGEND) &&
          !targetCatIds[pull.id] ? 1 : 0);
      }, 0);
    }

    function missingTargetCount(mask) {
      return popcount((targetMask & (~mask >>> 0)) >>> 0);
    }

    function remainingTargetCostLowerBound(mask) {
      var units = 0;
      for (var index = 0; index < targets.length; index += 1) {
        var bit = targetBit(index);
        if ((mask & bit) === bit) continue;
        units += targetMinCostUnits[index];
      }
      return units;
    }

    function remainingPullCapacity(state) {
      if (state.offset > maxStartOffset) return 0;
      var guaranteedSavings = Math.max(0,
        maxGuaranteed - state.guaranteedUsed);
      return Math.floor((maxStartOffset - state.offset + guaranteedSavings) / 2) + 1;
    }

    function paretoStatePruned(state) {
      var missingTargets = missingTargetCount(state.mask);
      var remainingCapacity = remainingPullCapacity(state);
      if (missingTargets > remainingCapacity) {
        paretoPrunedTargetCapacity += 1;
        return true;
      }
      var minRemainingCost = remainingTargetCostLowerBound(state.mask);
      if (state.costUnits + minRemainingCost > paretoBudgetUnits) {
        paretoPrunedTargetCost += 1;
        return true;
      }
      if (!best) return false;

      var spareBudget = paretoBudgetUnits - state.costUnits - minRemainingCost;
      var maxHarvestByBudget = Math.max(0,
        Math.floor(spareBudget / REGULAR_COST));
      var maxHarvestByDistance = Math.max(0,
        remainingCapacity - missingTargets);
      var maxFutureHarvest = Math.min(maxHarvestByBudget,
        maxHarvestByDistance);
      if (state.harvestDraws + maxFutureHarvest < best.harvestDraws) {
        paretoPrunedHarvestBound += 1;
        return true;
      }
      return false;
    }
    ...
    function dominates(lhs, rhs) {
      return lhs.alive && lhs.costUnits <= rhs.costUnits &&
        ...
        lhs.scheduleDay <= rhs.scheduleDay && maskHas(lhs.mask, rhs.mask) &&
        (optimization !== "pareto" || lhs.harvestDraws >= rhs.harvestDraws);
    }
    ...
    function addNode(attrs) {
      ...
      var node = {
        ...
        scheduleDay: attrs.scheduleDay,
        harvestDraws: attrs.harvestDraws,
    ...
    function consider(nodeId) {
      ...
      if (optimization === "pareto") {
        if (node.costUnits > paretoBudgetUnits) return;
        if (!best || node.harvestDraws > best.harvestDraws ||
            (node.harvestDraws === best.harvestDraws &&
              (node.costUnits < best.costUnits ||
                (node.costUnits === best.costUnits &&
                  (node.offset < best.offset ||
                    (node.offset === best.offset && node.order < best.order)))))) {
          best = {
            nodeId: nodeId,
            offset: node.offset,
            scoreUnits: node.costUnits,
            ...
            harvestDraws: node.harvestDraws,
            paretoBaseCostUnits: paretoBaseCostUnits,
            paretoAllowanceUnits: paretoAllowance,
            paretoBudgetUnits: paretoBudgetUnits,
            order: node.order
          };
        }
        return;
    ...
    function transition(stateId, state, attrs) {
      if (optimization === "pareto" && paretoStatePruned(attrs)) return;
      var nodeId = addNode({
        offset: attrs.offset,
        lastRareId: attrs.lastRareId,
        mask: attrs.mask,
    ...
    for (var offset = 0; offset <= maxStartOffset; offset += 1) {
      ...
      for (var bucketIndex = 0; bucketIndex < bucket.length; bucketIndex += 1) {
        ...
        if (optimization === "pareto" && paretoStatePruned(state)) continue;

        var normalTransitions = [];
        var normalSeen = Object.create(null);
        events.forEach(function (event) {
          var nextScheduleDay = scheduleDayFor(
            event, state.scheduleDay, scheduleAware);
          if (nextScheduleDay == null) return;
          ...
          var harvestGain = harvestCount([rolled]);
          var balanceDelta = optimization === "balance" ?
            actionCost(regularCost, counts, optimization) : 0;
          var dedupeKey = [rolled.nextOffset, rolled.lastRareId,
            targetResult.mask, nextScheduleDay,
            optimization === "pareto" ? harvestGain : 0,
            balanceDelta].join("|");
        ...
        var hasRerollAlternative = normalTransitions.length > 1 &&
        ...
        normalTransitions.forEach(function (item) {
          ...
          transition(stateId, state, {
            ...
            harvestDraws: state.harvestDraws + item.harvestGain,
            costUnits: state.costUnits + actionCost(regularCost, counts,
              optimization),
            action: {
              type: "roll",
              event: event.event,
              ...
              scheduleDay: item.scheduleDay,
              scheduleStartDay: event.startDay,
              scheduleEndDay: event.endDay,
              start: rolled.start,
        ...
        var proactiveTicketDefense = normalTransitions.some(function (item) {
          ...
          return events.some(function (nextEvent) {
            if (scheduleDayFor(nextEvent, item.scheduleDay, scheduleAware) == null) {
              return false;
            }
        ...
        tickets.forEach(function (ticket) {
          ...
          var ticketScheduleDay = scheduleDayFor(
            ticket, state.scheduleDay, scheduleAware);
          if (ticketScheduleDay == null) return;

          var baseTicketCost = ticketCost(ticket.kind);
          ...
          if (needsTicketDefense || proactiveTicketDefense ||
            ...
            transition(stateId, state, {
              ...
              scheduleDay: ticketScheduleDay,
              harvestDraws: state.harvestDraws + ticketHarvestGain,
              costUnits: state.costUnits + actionCost(baseTicketCost,
              ...
              action: {
                ...
                scheduleDay: ticketScheduleDay,
                scheduleStartDay: ticket.startDay,
                scheduleEndDay: ticket.endDay,
                ...
                harvestDraws: ticketHarvestGain,
                acquired: ticketTargets.acquired,
                cost: baseTicketCost / COST_SCALE
              }
            });
          }
        });
        ...
        if (state.guaranteedUsed < maxGuaranteed) {
          ...
          events.forEach(function (event) {
            ...
            var guaranteedScheduleDay = scheduleDayFor(
              event, state.scheduleDay, scheduleAware);
            if (guaranteedScheduleDay == null) return;
            ...
            var guaranteedHarvestGain = harvestCount(guaranteed.pulls);
            var guaranteedBalanceDelta = optimization === "balance" ?
              actionCost(GUARANTEED_COST, guaranteedCounts, optimization) : 0;
            var guaranteedKey = [guaranteed.nextOffset,
              guaranteed.lastRareId, guaranteedTargets.mask,
              guaranteedScheduleDay,
              optimization === "pareto" ? guaranteedHarvestGain : 0,
              guaranteedBalanceDelta].join("|");
            ...
            transition(stateId, state, {
              ...
              platinumUsed: state.platinumUsed,
              legendTicketUsed: state.legendTicketUsed,
              scheduleDay: guaranteedScheduleDay,
              harvestDraws: state.harvestDraws + guaranteedHarvestGain,
              ...
              action: {
                ...
                harvestDraws: guaranteedHarvestGain,
    ...
    var partial = limitHit && optimization === "pareto" && !!best;
    if (limitHit && !partial) {
      return {
        status: "limit",
    ...
    var baseCostUnits = regularUses * regularCost +
      best.platinumUsed * PLATINUM_TICKET_COST +
      best.legendTicketUsed * LEGEND_TICKET_COST +
    ...
    return {
      ...
      message: partial ?
        "상태 한도에 도달해 현재까지 찾은 최선 경로를 표시합니다. 최적성은 보장되지 않습니다." :
        null,
      destination: positionLabel(best.offset),
      nextOffset: best.offset,
      optimization: optimization,
      cost: best.scoreUnits / COST_SCALE,
      scoreUnits: best.scoreUnits,
      costUnits: best.costUnits,
      rawCost: best.costUnits / COST_SCALE,
      baseCost: baseCostUnits / COST_SCALE,
      regularUnitCost: regularCost / COST_SCALE,
      balanceCredit: balanceCreditUnits / COST_SCALE,
      harvestDraws: best.harvestDraws || 0,
      paretoBaseCost: best.paretoBaseCostUnits == null ? null :
        best.paretoBaseCostUnits / COST_SCALE,
      paretoAllowance: best.paretoAllowanceUnits == null ? null :
        best.paretoAllowanceUnits / COST_SCALE,
      paretoBudget: best.paretoBudgetUnits == null ? null :
        best.paretoBudgetUnits / COST_SCALE,
      paretoPrunedTargetCost: paretoPrunedTargetCost,
      paretoPrunedTargetCapacity: paretoPrunedTargetCapacity,
      paretoPrunedHarvestBound: paretoPrunedHarvestBound,
      uberDraws: uberDraws,
