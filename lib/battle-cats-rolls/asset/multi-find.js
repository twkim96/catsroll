(function (global) {
  ...
  function renderResult(result) {
    ...
    var costSummary = result.optimization === "balance" ?
      '<p>레어티켓 ' + result.regularUses + '회 × 0.02 + 플래티넘 티켓 ' +
        (result.platinumUses || 0) + '회 × 1 + 레전드 티켓 ' +
        (result.legendTicketUses || 0) + '회 × 2 + 확뽑 ' + result.guaranteedUses +
        '회 × 1 = 기본 코스트 ' + formatCost(result.baseCost) + ' - 울슈레 ' +
        result.uberDraws + '회 × 0.14 - 레전드 ' + result.legendDraws +
        '회 × 0.14 = 탐욕 코스트 ' + formatCost(result.cost) +
    ...
    showResult("완료", "", '<div class="multi-find-result-heading">' +
      ...
      (result.optimization === "distance" ? '최단거리' :
        (result.optimization === "balance" ? '탐욕' : '최소코스트')) + ' · ' +
