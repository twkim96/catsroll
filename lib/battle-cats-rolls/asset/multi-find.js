(function (global) {
  ...
  var seriesCatalog = Array.isArray(data.find_series) ? data.find_series : [];
  ...
  var els = {
    ...
    schedule: doc.getElementById("multi_find_schedule"),
    ...
    reset: doc.getElementById("multi_find_reset"),
    ...
    resultStatus: doc.getElementById("multi_find_result_status"),
    resultContent: doc.getElementById("multi_find_result_content"),
  ...
  if (!els.optimization || !els.schedule || !els.maxPlatinum ||
      !els.maxLegendTicket || !els.reset || !els.dialog || !els.result ||
      !els.resultStatus || !els.resultContent) return;
  ...
  function showResult(status, className, html) {
    els.result.hidden = false;
    els.result.className = "multi-find-result" +
      (className ? " " + className : "");
    els.resultStatus.textContent = status ? "· " + status : "";
  ...
  catalog = catalog.map(function (cat) {
    ...
    return cat.id > 0 && [2, 3, 4, 5].indexOf(cat.rarity) !== -1;
  });
      ...
      optimization: optimization === "distance" ? "distance" :
        (optimization === "balance" ? "balance" :
          (optimization === "pareto" ? "pareto" :
            (optimization === "cost40" ? "cost40" :
              (optimization === "cost50" ? "cost50" : "cost")))),
      maxGuaranteed: clamp(parseInt(
        saved.maxGuaranteed == null ? saved.max_guaranteed : saved.maxGuaranteed,
        10) || 0, 0, 20),
      scheduleAware: saved.scheduleAware === true || saved.schedule_aware === true ||
        saved.scheduleAware === 1 || saved.schedule_aware === 1,
  ...
  function save() {
    ...
    try {
      global.localStorage.setItem(storageKey, JSON.stringify({
        ...
        schedule_aware: settings.scheduleAware,
  ...
  function selectedPoolIds() {
    ...
    latestSnapshot.rows.forEach(function (row) {
      rowPoolCatIds(row).forEach(function (id) { result[id] = true; });
    });
    return result;
  }

  function eventInfo(lang, eventName) {
    var region = data.regions[lang] || {};
    var events = Array.isArray(region.events) ? region.events : [];
  ...
  function seriesInfo(id) {
    for (var index = 0; index < seriesCatalog.length; index += 1) {
      if (parseInt(seriesCatalog[index].id, 10) === id) return seriesCatalog[index];
    }
    return null;
  }

  function rowPoolCatIds(row) {
    var result = [];
    if (!row.pool || !row.pool.slots) return result;
    [2, 3, 4, 5].forEach(function (rarity) {
      var slots = row.pool.slots[rarity] || row.pool.slots[String(rarity)] || [];
      slots.forEach(function (id) { if (id > 0) result.push(id); });
    });
    return result;
  }

  function selectedBannerGroups() {
    if (!latestSnapshot || !Array.isArray(latestSnapshot.rows)) return [];
    return latestSnapshot.rows.map(function (row, index) {
      var event = eventInfo(row.lang, row.event);
      var seriesId = event && event.series_id != null ?
        parseInt(event.series_id, 10) : -1;
      var series = seriesInfo(seriesId) || {};
      return {
        id: seriesId,
        key: "selected:" + index,
        label: row.title || row.label || series.label || row.event,
        aliases: [row.label, series.label].concat(series.aliases || []),
        cat_ids: rowPoolCatIds(row)
      };
    });
  }
  ...
  function renderSummary() {
    ...
    els.schedule.classList.toggle("is-active", settings.scheduleAware);
    els.schedule.setAttribute("aria-pressed", settings.scheduleAware ? "true" : "false");
    ...
    els.reset.disabled = names.length === 0;
  }
  ...
  function matchesSearch(cat, group, query, tokens) {
    if (!query) return true;
    if (global.MultiFindCatalog &&
        typeof global.MultiFindCatalog.matches === "function") {
      return global.MultiFindCatalog.matches(group, cat, query);
    }
    return tokens.every(function (token) {
      return cat.search.indexOf(token) !== -1;
    }) || cat.searchCompact.indexOf(query.replace(/\s+/g, "")) !== -1;
  }

  function seriesGroupHtml(group, cats, inPool) {
    return [
      '<section class="multi-find-series-group',
        group.selected ? ' is-selected-series' : '', '">',
      '<div class="multi-find-series-heading">',
        '<h4>', h(group.label), '</h4>',
        group.selected ? '<span class="multi-find-series-selected">선택 배너</span>' : '',
        '<span class="multi-find-series-count">', cats.length, '</span>',
      '</div>',
      '<div class="multi-find-series-tags">',
        cats.map(function (cat) { return tagFor(cat, !!inPool[cat.id]); }).join(""),
      '</div>',
      '</section>'
    ].join("");
  }
  ...
  function renderAvailable() {
    ...
    var organizer = global.MultiFindCatalog;
    var groups = organizer && typeof organizer.organize === "function" ?
      organizer.organize(catalog, seriesCatalog, selectedBannerGroups()) : [{
        label: "캐릭터",
        selected: false,
        cats: catalog.slice().sort(function (left, right) {
          return left.name.localeCompare(right.name, "ko");
        })
      }];
    var visibleCount = 0;
    els.available.innerHTML = groups.map(function (group) {
      var cats = group.cats.filter(function (cat) {
        return matchesSearch(cat, group, query, tokens);
      });
      if (!cats.length) return "";
      visibleCount += cats.length;
      return seriesGroupHtml(group, cats, inPool);
    }).join("");
  ...
  function showPending() {
    ...
    showResult("반영 대기", "", '<p>변경 사항을 배열과 경로에 반영하려면 ' +
      '<button type="button" class="multi-find-inline-apply" ' +
      'data-action="apply-settings" aria-label="변경 사항 바로 적용">' +
      'Apply</button>를 누르세요.</p>' +
  ...
  function resetTargets() {
    if (!settings.targets.length) return;
    settings.targets = [];
    settingsChanged();
    renderDialog();
  }
  ...
  function renderRunning() {
    clearRouteMarks();
    lastSuccessfulResult = null;
    showResult("계산 중", "is-running", '<p>계산 중… ' +
  ...
  function clearRouteMarks(container) {
    container = container || els.tables;
    if (!container) return;
    Array.prototype.forEach.call(
      container.querySelectorAll(".multi-route-marker-stack"),
      function (marker) { marker.remove(); }
    );
    Array.prototype.forEach.call(
      container.querySelectorAll(".multi-route-marked"),
      function (cell) {
  ...
  function routeScheduleValid(actions) {
    if (!settings.scheduleAware) return true;
    var currentDay = null;
    return (actions || []).every(function (action) {
      if (action.scheduleDay == null || action.scheduleStartDay == null ||
          action.scheduleEndDay == null) return false;
      var day = Number(action.scheduleDay);
      var startDay = Number(action.scheduleStartDay);
      var endDay = Number(action.scheduleEndDay);
      if (!Number.isFinite(day) || !Number.isFinite(startDay) ||
          !Number.isFinite(endDay) || endDay <= startDay) return false;
      if (day < startDay || day >= endDay) return false;
      if (currentDay != null && day < currentDay) return false;
      currentDay = day;
      return true;
    });
  }

  function applyRouteMarks(actions, container) {
    container = container || els.tables;
    clearRouteMarks(container);
    if (!container || !actions || !actions.length) return;
    if (!routeScheduleValid(actions)) return;
    ...
    Array.prototype.forEach.call(
      container.querySelectorAll("[data-route-event][data-route-position]"),
      function (cell) {
    ...
    Array.prototype.forEach.call(
      container.querySelectorAll("[data-route-number-position]"),
      function (cell) {
  ...
  function routeIsDrawn() {
    var button = els.result.querySelector("button[data-action='draw-route']");
  ...
  function routeLines(actions, optimization) {
    ...
    function balanceBonus(action) {
      ...
      var parts = [];
      if (uber) parts.push("울슈레 " + uber + "회 × 0.14");
      if (legend) parts.push("레전드 " + legend + "회 × 0.14");
      return ' · 탐욕 보너스 -(' + h(parts.join(" + ")) + ')';
    }
  ...
  function formatCost(value) {
    return Number(value || 0).toFixed(3).replace(/\.000$/, "")
      .replace(/(\.\d\d)0$/, "$1").replace(/(\.\d)00$/, "$1");
  }

  function renderResult(result) {
    clearRouteMarks();
    var partial = !!result && result.status === "partial";
    if (!result || (result.status !== "success" && !partial)) {
      lastSuccessfulResult = null;
      var label = result && result.status === "limit" ? "계산 범위 초과" :
        (result && result.status === "invalid" ? "입력 확인 필요" : "불가능");
      showResult(label, "is-error", '<p>' +
    ...
    if (!routeScheduleValid(result.actions || [])) {
      lastSuccessfulResult = null;
      showResult("일정 경로 오류", "is-error",
        '<p>배너 일정 순서와 맞지 않는 경로가 감지되어 계획표를 표시하지 않았습니다. ' +
        'Apply를 눌러 다시 계산해 주세요.</p>');
      return;
    }
    ...
    var regularUnitCost = result.regularUnitCost == null ? 0.02 :
      Number(result.regularUnitCost);
    var resourceFormula = '레어티켓 ' + result.regularUses +
      '회 × ' + formatCost(regularUnitCost) + ' + 플래티넘 티켓 ' +
      (result.platinumUses || 0) +
      '회 × 1 + 레전드 티켓 ' + (result.legendTicketUses || 0) +
      '회 × 2 + 확뽑 ' + result.guaranteedUses + '회 × 1';
    var costSummary = result.optimization === "balance" ?
      '<p>' + resourceFormula + ' = 기본 코스트 ' + formatCost(result.baseCost) +
        ' - 울슈레 ' + result.uberDraws + '회 × 0.14 - 레전드 ' +
        result.legendDraws + '회 × 0.14 = 탐욕 코스트 ' +
        formatCost(result.cost) + (Number(result.rawCost) < 0 ?
          ' (0점 하한 적용)' : '') + '</p>' :
      (result.optimization === "pareto" ?
        '<p>' + resourceFormula + ' = 실제 코스트 ' + formatCost(result.cost) +
          ' · 파레토 예산 ' + formatCost(result.paretoBaseCost) + ' + ' +
          formatCost(result.paretoAllowance) + ' = ' +
          formatCost(result.paretoBudget) + ' · 목표 외 울슈레·레전드레어 ' +
          (result.harvestDraws || 0) + '회</p>' :
        '<p>' + resourceFormula + ' = 총 코스트 ' +
          formatCost(result.cost) + '</p>');
    lastSuccessfulResult = result;
    var partialNotice = partial ?
      '<p class="multi-find-partial-note"><strong>최적성 미확인:</strong> ' +
        h(result.message || "상태 한도까지 찾은 최선 경로입니다.") + '</p>' : '';
    showResult(partial ? "부분 결과" : "완료", partial ? "is-partial" : "",
      '<div class="multi-find-result-heading">' +
      '<button type="button" class="multi-find-route-draw"' +
      ' data-action="draw-route" aria-pressed="false">계획표 그리기</button>' +
      '</div>' + partialNotice + route +
      ...
      (settings.scheduleAware ? '일정 순서 · ' : '') +
      (result.optimization === "distance" ? '최단거리' :
        (result.optimization === "balance" ? '탐욕' :
          (result.optimization === "pareto" ? '파레토' :
            (result.optimization === "cost40" ? '최소코스트(40)' :
              (result.optimization === "cost50" ? '최소코스트(50)' :
                '최소코스트'))))) + ' · ' +
  ...
  function selectedTicketPools(snapshot, region, lang) {
    ...
    snapshot.rows.forEach(function (row) {
      ...
      var info = eventInfo(row.lang, row.event) || {};
      byKind[kind].push({
        lang: row.lang,
        event: row.event,
    ...
    ["platinum", "legend"].forEach(function (kind) {
      ...
      result.push({
        ...
        kind: kind,
        start_on: fallback.start_on,
        end_on: fallback.end_on,
        pool: fallback.pool
      });
    });
    return result;
  ...
  function startSearch(snapshot) {
    ...
    var input = {
      ...
      scheduleAware: settings.scheduleAware,
  ...
  els.optimization.addEventListener("change", function () {
    settings.optimization = els.optimization.value === "distance" ? "distance" :
      (els.optimization.value === "balance" ? "balance" :
        (els.optimization.value === "pareto" ? "pareto" :
          (els.optimization.value === "cost40" ? "cost40" :
            (els.optimization.value === "cost50" ? "cost50" : "cost"))));
    settingsChanged();
  });
  ...
  els.schedule.addEventListener("click", function () {
    settings.scheduleAware = !settings.scheduleAware;
    settingsChanged();
  });
  ...
  els.reset.addEventListener("click", resetTargets);
  ...
  els.result.addEventListener("click", function (event) {
    var inlineApply = event.target.closest(
      "button[data-action='apply-settings']");
    if (inlineApply) {
      var mainApply = doc.getElementById("multi_apply");
      if (mainApply && !mainApply.disabled) mainApply.click();
      return;
    }
  ...
  root.addEventListener("multi-track:window-updated", function () {
    if (routeIsDrawn() && lastSuccessfulResult) {
      applyRouteMarks(lastSuccessfulResult.actions || []);
    }
  });
  ...
  global.MultiFindApp = {
    getTargetIds: function () {
      return settings.targets.map(function (target) { return target.cat_id; });
    },
    getTargetCats: function () {
      return settings.targets.map(function (target) {
        var cat = catalogById[target.cat_id];
        return cat ? { id: cat.id, name: cat.name, rarity: cat.rarity } : null;
      }).filter(Boolean);
    },
    getNonRegularRareIds: function () {
      return catalog.filter(function (cat) { return cat.rarity === 2; }).map(
        function (cat) { return cat.id; });
    },
    getNonRegularSuperRareIds: function () {
      return catalog.filter(function (cat) { return cat.rarity === 3; }).map(
        function (cat) { return cat.id; });
    },
    ...
    decorateRouteMarks: function (container) {
      if (routeIsDrawn() && lastSuccessfulResult) {
        applyRouteMarks(lastSuccessfulResult.actions || [], container);
      }
    }
  };
