(function (global) {
  ...
  function selectedSeriesIds() {
    ...
    latestSnapshot.rows.forEach(function (row) {
      ...
      if (!event || event.series_id == null) return;
      var id = parseInt(event.series_id, 10);
  ...
  function matchesSearch(cat, query, compact, tokens) {
  ...
  function seriesGroupHtml(group, cats, inPool) {
    return [
      ...
      '<div class="multi-find-series-heading">',
        '<h4>', h(group.label), '</h4>',
        group.selected ? '<span class="multi-find-series-selected">선택 배너 포함</span>' : '',
  ...
  function renderAvailable() {
    ...
    var groups = organizer && typeof organizer.organize === "function" ?
      organizer.organize(catalog, seriesCatalog, selectedSeriesIds(),
        Object.keys(inPool)) : [{
        label: "캐릭터",
        selected: false,
    ...
    els.available.innerHTML = groups.map(function (group) {
      var cats = group.cats.filter(function (cat) {
        return matchesSearch(cat, query, compact, tokens);
      });
