...
(function (global) {
  ...
  // ---- coloring (mirrors color_rarity / color_label / color_guaranteed) ----
  TrackRender.prototype.colorRarity = function (cat) {
    var label = cat.rarityLabel();
    if (label === 'legend') return 'legend';
    if (cat.id === this.ctx.find) return 'found';
    if (EX_SET[cat.id]) return 'exclusive';
    return label;
  };
  ...
  TrackRender.prototype.colorLabel = function (cat, type, rerolled) {
    ...
    var rarity = this.colorRarity(cat);
    var parts = [cursor, rarity];
    if (this.ownedSet[cat.id]) parts.push('owned');
  ...
  TrackRender.prototype.td = function (cat, type, o) {
    ...
    return '<td rowspan="' + rowspan + '" class="' + type + ' ' +
  ...
  global.TrackRender = { renderTable: renderTable, TrackRender: TrackRender, h: h };
