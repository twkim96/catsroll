(function (root, factory) {
  ...
  var els = {
    ...
    log: doc.getElementById("multi_plan_log"),
    logCount: doc.getElementById("multi_plan_log_count"),
    logList: doc.getElementById("multi_plan_log_list"),
    logEmpty: doc.getElementById("multi_plan_log_empty"),
    routeDetails: doc.getElementById("multi_plan_route"),
    routeCount: doc.getElementById("multi_plan_route_count"),
    routeList: doc.getElementById("multi_plan_route_list"),
    routeEmpty: doc.getElementById("multi_plan_route_empty"),
    infoName: doc.getElementById("multi_plan_info_name"),
    infoStatus: doc.getElementById("multi_plan_info_status"),
    infoReset: doc.getElementById("multi_plan_info_reset"),
    infoSave: doc.getElementById("multi_plan_info_save"),
  ...
  if (!els.mode || !els.load || !els.name || !els.reset || !els.save ||
      ...
      !els.log || !els.logCount || !els.logList || !els.logEmpty ||
      !els.routeDetails || !els.routeCount || !els.routeList ||
      !els.routeEmpty ||
      !els.infoName || !els.infoStatus || !els.infoReset || !els.infoSave ||
  ...
  function setStatus(message, error) {
    ...
    updatePlanInfoStatus(error ? message : "");
  }

  function currentPlanProgress() {
    var snapshot = trackApp.getFindSnapshot && trackApp.getFindSnapshot();
    return helpers.planProgressSummary(snapshot, routePlan, marks);
  }

  function setPlanName(value) {
    var name = String(value == null ? "" : value).slice(0, 80);
    els.name.value = name;
    els.infoName.value = name;
  }

  function updatePlanInfoStatus(errorMessage) {
    var detail = String(errorMessage || "").trim();
    if (!detail) {
      detail = currentPlanId && !hasUnsavedDraft() ?
        "‘" + String(els.name.value || "").trim() +
          "’ 플랜에 저장되어 있습니다." :
        "아직 플랜에 저장되지 않았습니다.";
    }
    els.infoStatus.textContent = currentPlanProgress() + " · " + detail;
    els.infoStatus.classList.toggle("is-error", !!errorMessage);
  }

  function syncSidebarPlanButton() {
    var button = doc.getElementById("recent-seeds-plan");
    if (!button) return;
    button.hidden = !els.mode.checked;
    button.setAttribute("aria-expanded", els.catsDialog.open ? "true" : "false");
  }
  ...
  function findTargetIds() {
    return helpers.normalizeTargetCatIds(root.MultiFindApp &&
      typeof root.MultiFindApp.getTargetIds === "function" ?
      root.MultiFindApp.getTargetIds() : []);
  }

  function findTargetCats() {
    return root.MultiFindApp &&
      typeof root.MultiFindApp.getTargetCats === "function" ?
      root.MultiFindApp.getTargetCats() : [];
  }

  function nonRegularRareIds() {
    return helpers.normalizeTargetCatIds(root.MultiFindApp &&
      typeof root.MultiFindApp.getNonRegularRareIds === "function" ?
      root.MultiFindApp.getNonRegularRareIds() : []);
  }

  function nonRegularSuperRareIds() {
    return helpers.normalizeTargetCatIds(root.MultiFindApp &&
      typeof root.MultiFindApp.getNonRegularSuperRareIds === "function" ?
      root.MultiFindApp.getNonRegularSuperRareIds() : []);
  }

  function selectedCatsGroupHtml(rarity, label, className, targetIds) {
    ...
    return [
      ...
      '<div class="multi-plan-cats-tags">', entries.map(function (entry) {
        var isTarget = targetIds.indexOf(entry.id) !== -1;
        return '<span class="multi-plan-cat-tag ' + className + '" title="' +
          h(entry.name + (isTarget ? " · 목표 캐릭터" : "")) + '">' +
          '<span>' + h(entry.name) + '</span>' +
          (entry.count > 1 ? '<span class="multi-plan-cat-count">×' +
            entry.count + '</span>' : '') +
          (isTarget ? '<span class="multi-plan-cat-target">목표</span>' : '') +
          '</span>';
      }).join(""), '</div></section>'
    ].join("");
  }

  function missingTargetsGroupHtml(entries) {
    if (!entries.length) return "";
    return [
      '<section class="multi-plan-cats-group multi-plan-missing-targets">',
      '<div class="multi-plan-cats-group-header"><h3>못 먹은 목표</h3>',
      '<span>', entries.length, '명</span></div>',
      '<div class="multi-plan-cats-tags">', entries.map(function (entry) {
        var rarityClass = entry.rarity === 5 ? "is-legend" :
          (entry.rarity === 4 ? "is-uber" :
            (entry.rarity === 3 ? "is-supa" : "is-rare"));
        return '<span class="multi-plan-cat-tag is-missing-target ' +
          rarityClass + '" title="' + h(entry.name) +
          ' · 현재 플랜에서 미획득"><span>' + h(entry.name) +
          '</span></span>';
      }).join(""), '</div></section>'
    ].join("");
  }
  ...
  function renderSelectedCatsDialog() {
    var targetIds = findTargetIds();
    var missingTargets = helpers.missingTargetCats(
      findTargetCats(), selectedCatEntries);
    els.catsList.innerHTML = selectedCatsGroupHtml(
      5, "레전드레어", "is-legend", targetIds) + selectedCatsGroupHtml(
      4, "울트라 슈퍼 레어", "is-uber", targetIds) + selectedCatsGroupHtml(
      3, "슈퍼레어", "is-supa", targetIds) + selectedCatsGroupHtml(
      2, "레어", "is-rare", targetIds) +
      missingTargetsGroupHtml(missingTargets);
    els.catsEmpty.hidden = selectedCatEntries.length > 0 ||
      missingTargets.length > 0;
    renderPlanRoutes();
  }

  function appendPlanRouteEntry(list, entry) {
    var item = doc.createElement("li");
    var position = doc.createElement("span");
    position.className = "multi-find-route-position";
    position.textContent = entry.position;
    item.appendChild(position);
    item.appendChild(doc.createTextNode(" · "));
    (entry.segments || []).forEach(function (segment) {
      if (!segment.cat) {
        item.appendChild(doc.createTextNode(segment.text || ""));
        return;
      }
      var cat = doc.createElement("span");
      cat.className = "multi-plan-route-cat";
      cat.textContent = segment.label || segment.cat.name || "";
      if (segment.target) {
        var targetBadge = doc.createElement("span");
        targetBadge.className = "multi-plan-route-badge is-target";
        targetBadge.textContent = "목표";
        cat.appendChild(targetBadge);
      }
      if (segment.selected) {
        var selectedBadge = doc.createElement("span");
        selectedBadge.className = "multi-plan-route-badge is-selected";
        selectedBadge.textContent = "선택";
        cat.appendChild(selectedBadge);
      }
      item.appendChild(cat);
    });
    if (entry.next) {
      item.appendChild(doc.createTextNode(" → " + entry.next));
    }
    list.appendChild(item);
  }

  function renderPlanRouteList(list, empty, count, entries) {
    list.innerHTML = "";
    entries.forEach(function (entry) { appendPlanRouteEntry(list, entry); });
  ...
  function renderPlanRoutes() {
    var snapshot = trackApp.getFindSnapshot && trackApp.getFindSnapshot();
    var entries = helpers.planRouteEntries(
      snapshot, routePlan, marks, findTargetIds());
    renderPlanRouteList(els.logList, els.logEmpty, els.logCount, entries);
    renderPlanRouteList(
      els.routeList, els.routeEmpty, els.routeCount, entries);
    els.routeDetails.hidden = marks.length === 0;
  }
  ...
  function renderSelectedCats() {
    ...
    var allowedRareIds = nonRegularRareIds();
    var allowedSuperRareIds = nonRegularSuperRareIds();
    selectedCatEntries = helpers.catSummaryEntries(
      cats, allowedSuperRareIds, allowedRareIds);
    var summary = helpers.summarizeCats(cats, allowedSuperRareIds, allowedRareIds);
    els.selectedCats.textContent = summary || "선택 없음";
    els.selectedCats.title = summary ||
      "선택된 비정규 레어·슈퍼레어·울슈레·레전드레어가 없습니다.";
    els.selectedCats.disabled = false;
    els.selectedCats.classList.toggle("is-empty", !summary);
    els.selectedCats.setAttribute("aria-label", summary ?
      "선택 비정규 레어·슈퍼레어·울슈레·레전드레어 전체 보기: " + summary :
      "선택된 비정규 레어·슈퍼레어·울슈레·레전드레어가 없습니다.");
    renderSelectedCatsDialog();
  }

  function openSelectedCatsDialog() {
    if (els.catsDialog.open) return;
    renderSelectedCatsDialog();
    els.infoName.value = els.name.value;
    updatePlanInfoStatus();
    if (typeof els.catsDialog.showModal === "function") {
      els.catsDialog.showModal();
    } else {
      els.catsDialog.setAttribute("open", "");
    }
    syncSidebarPlanButton();
  }
  ...
  function markKindLabel(mark) {
    if (mark.kind === "reroll") return "R 열변경";
    if (mark.kind === "guaranteed") {
      return mark.variant === "rerolled" ? "RG 확정뽑기" : "확정뽑기";
    }
    return "일반 뽑기";
  }

  function markRemovalLabel(mark) {
    var snapshot = trackApp.getFindSnapshot && trackApp.getFindSnapshot();
    var row = snapshot && snapshot.rows && snapshot.rows[mark.column];
    var banner = row && (row.label || row.title) || "배너 " + (mark.column + 1);
    var selector = 'td[data-plan-column-index="' + mark.column +
      '"][data-route-position="' + mark.position + '"]';
    var cell = els.tables.querySelector(selector);
    var line = null;
    if (cell) {
      var lineSelector = '[data-plan-pick-kind="' + mark.kind + '"]';
      if (mark.kind === "guaranteed") {
        lineSelector += '[data-plan-pick-variant="' +
          (mark.variant || "base") + '"]';
      }
      line = cell.querySelector(lineSelector);
    }
    var picked = typeof trackApp.getPlanMarkLabel === "function" ?
      trackApp.getPlanMarkLabel(mark) : "";
    if (!picked && line) {
      picked = String(line.textContent || "").replace(/\s+/g, " ").trim();
    }
    if (!picked) picked = markKindLabel(mark);
    return "• " + mark.position + " · " + banner + " · " + picked;
  }

  function finishMarkChange(prunedCount) {
    ...
    setStatus((prunedCount ? "앞선 선택 취소로 도달할 수 없는 뒤쪽 좌표 " +
      prunedCount + "개도 해제했습니다. · " : "") +
      currentPlanProgress() + " · 아직 플랜에 저장되지 않았습니다.");
    syncSharedPlanState();
  }
  ...
  function toggleMark(cell, kind, variant) {
    ...
    if (index === -1) {
      nextMarks = nextMarks.filter(function (item) {
    ...
    var nextRoutePlan = calculateRoutePlan(nextMarks,
      previousRoutePlan.valid ? previousRoutePlan.auto : [], {
        pruneInvalid: removing
      });
    if (removing && nextRoutePlan.ready && nextRoutePlan.pruned.length) {
      var prunedMarks = nextRoutePlan.pruned.slice();
      var pruned = Object.create(null);
      prunedMarks.forEach(function (item) {
        pruned[markKey(item)] = true;
      });
      var cleanedMarks = nextMarks.filter(function (item) {
        return !pruned[markKey(item)];
      });
      openConfirm({
        title: "뒤쪽 선택 함께 해제",
        message: "해제할 선택\n" + markRemovalLabel(mark) +
          "\n\n함께 사라지는 선택 " + prunedMarks.length + "개\n" +
          prunedMarks.map(markRemovalLabel).join("\n") +
          "\n\n이대로 함께 해제할까요?",
        acceptLabel: "함께 해제",
        danger: true,
        accept: function () {
          marks = cleanedMarks;
          routePlan = calculateRoutePlan(marks,
            nextRoutePlan.valid ? nextRoutePlan.auto : []);
          finishMarkChange(prunedMarks.length);
        }
      });
      return;
    }
    if (index === -1 && (!nextRoutePlan.ready || nextRoutePlan.invalid)) {
      var rejectedRoutePlan = nextRoutePlan;
      var specialHint = rejectedRoutePlan.ready ?
        calculateSpecialTicketHint(nextMarks,
          previousRoutePlan.valid ? previousRoutePlan.auto : []) : "";
      decorateMarks(els.tables);
  ...
  function loadPlan(id) {
    ...
    rememberSavedDraft();
    closeDialog();
    setStatus("격리된 플랜을 불러왔습니다. · " + currentPlanProgress() +
      " · 변경 내용은 플랜 저장을 누를 때만 반영됩니다.");
    syncSharedPlanState();
  }
  ...
  function persistPlan(name, track, existingId) {
    ...
    decorateMarks(els.tables);
    rememberSavedDraft();
    setStatus("‘" + saved.plan.name + "’ 저장 완료 · " +
      currentPlanProgress());
    syncSharedPlanState();
  ...
  function savePlan(focusTarget) {
    var name = String(els.name.value || "").trim();
    if (!name) {
      setStatus("플랜명을 입력해 주세요.", true);
      (focusTarget || els.name).focus();
      return;
    }
    var track = trackApp.getShareState && trackApp.getShareState();
    if (!track) {
  ...
  function applySharedPlan(shared) {
    ...
    routePlan = helpers.emptyRoutePlan();
    savedDraftSignature = null;
    setPlanName(shared.name);
    els.mode.checked = true;
    syncPlanMode();
    refreshRoutePlan();
    decorateMarks(els.tables);
    setStatus("공유받은 임시 플랜입니다. · " + currentPlanProgress() +
      " · 플랜 저장을 누르기 전에는 " +
  ...
  els.save.addEventListener("click", function () { savePlan(els.name); });
  els.infoReset.addEventListener("click", requestPlanReset);
  els.infoSave.addEventListener("click", function () { savePlan(els.infoName); });
  els.name.addEventListener("input", function () {
    els.infoName.value = els.name.value;
    updatePlanInfoStatus();
  });
  els.name.addEventListener("change", syncSharedPlanState);
  els.infoName.addEventListener("input", function () {
    els.name.value = els.infoName.value;
    updatePlanInfoStatus();
  });
  els.infoName.addEventListener("change", syncSharedPlanState);
  ...
  root.MultiPlanApp = {
    ...
    openInfo: openSelectedCatsDialog,
  ...
  var maxPlans = 100;
  var regularRouteCost = 2;
  var guaranteedRouteCost = 100;
  var platinumTicketRouteCost = 100;
  var legendTicketRouteCost = 200;
  ...
  function normalizeTrack(track) {
    ...
    return {
      ...
      count: integer(track.count, 100, 1, 999),
  ...
  function normalizeMark(mark, track) {
    ...
    if (!Number.isFinite(sequence) || sequence < 1 || sequence > 999) return null;
  ...
  function catSummaryEntries(cats, allowedSuperRareIds, allowedRareIds) {
    var entries = [];
    var byKey = Object.create(null);
    var allowedRare = Object.create(null);
    var allowedSuperRare = Object.create(null);
    normalizeTargetCatIds(allowedRareIds).forEach(function (id) {
      allowedRare[id] = true;
    });
    normalizeTargetCatIds(allowedSuperRareIds).forEach(function (id) {
      allowedSuperRare[id] = true;
    });
    (Array.isArray(cats) ? cats : []).forEach(function (cat) {
      ...
      if (!name || ([4, 5].indexOf(rarity) === -1 &&
          !(rarity === 3 && allowedSuperRare[id]) &&
          !(rarity === 2 && allowedRare[id]))) return;
  ...
  function summarizeCats(cats, allowedSuperRareIds, allowedRareIds) {
    return catSummaryEntries(cats, allowedSuperRareIds, allowedRareIds).map(function (entry) {
      return entry.name + (entry.count > 1 ? " ×" + entry.count : "");
    }).join(", ");
  }

  function missingTargetCats(targets, acquiredEntries) {
    var acquired = Object.create(null);
    (Array.isArray(acquiredEntries) ? acquiredEntries : []).forEach(
      function (entry) {
        var id = parseInt(entry && entry.id, 10);
        if (id > 0) acquired[id] = true;
      });
    var seen = Object.create(null);
    return (Array.isArray(targets) ? targets : []).map(function (target) {
      var rawId = target && (target.id == null ? target.cat_id : target.id);
      var id = parseInt(rawId, 10);
      var name = limitedText(target && target.name, 160).trim();
      var rarity = parseInt(target && target.rarity, 10);
      if (!(id > 0) || !name || [2, 3, 4, 5].indexOf(rarity) === -1 ||
          acquired[id] || seen[id]) return null;
      seen[id] = true;
      return { id: id, name: name, rarity: rarity };
    }).filter(Boolean).sort(function (left, right) {
      return right.rarity - left.rarity ||
        left.name.localeCompare(right.name, "ko") || left.id - right.id;
    });
  }

  function normalizeTargetCatIds(values) {
    var seen = Object.create(null);
  ...
  function routeTicketKind(pool) {
    if (!pool || !pool.platinum) return "";
    return String(pool.platinum).toLowerCase() === "legend" ?
      "legend" : "platinum";
  }

  function regularRouteAction(row, rolled, position, selected, avoidedR) {
    return {
      type: "roll",
      column: row.column,
      position: position,
      resultLabel: String(rolled.resultLabel || position),
      next: String(rolled.next || ""),
      kind: rolled.rerolled ? "reroll" : "regular",
      selected: selected === true,
      avoidedR: avoidedR === true,
      ticketKind: routeTicketKind(row.pool),
      originalName: String(rolled.originalName || ""),
      cat: routeCat(rolled)
    };
  }

  function guaranteedRouteAction(row, guaranteed, mark) {
    var guaranteedPull = (guaranteed.pulls || []).filter(function (pull) {
      return pull && pull.guaranteed;
    })[0];
    var guaranteedLabel = String(guaranteed.guaranteedLabel ||
      guaranteedPull && (guaranteedPull.resultLabel || guaranteedPull.start) ||
      mark.position + "G");
    return {
      type: "guaranteed",
      column: row.column,
      ...
      next: String(guaranteed.next || ""),
      selected: true,
      variant: /RG$/.test(guaranteedLabel) ?
        "rerolled" : "base",
      pulls: (guaranteed.pulls || []).map(function (pull) {
        return {
          position: String(pull.resultLabel || pull.start || mark.position),
          guaranteed: pull.guaranteed === true,
          cat: routeCat(pull)
        };
      })
  ...
  function regularActionCost(pool) {
    var kind = routeTicketKind(pool);
    if (kind === "legend") return legendTicketRouteCost;
    if (kind === "platinum") return platinumTicketRouteCost;
    return regularRouteCost;
  }
    ...
    for (var offset = 0; offset < targetOffset; offset += 1) {
      ...
      Object.keys(bucket).forEach(function (stateKey) {
        ...
        rows.forEach(function (row) {
          ...
          simulated.push({ row: row, rolled: rolled });
        });
        var hasRerollAlternative = simulated.some(function (item) {
          return item.rolled && item.rolled.rerolled;
        });
        simulated.forEach(function (item) {
          ...
          var routeAction = regularRouteAction(row, rolled, position, false,
            !rolled.rerolled && hasRerollAlternative);
  ...
  function applyRouteMark(engine, snapshot, candidate, mark, order) {
    ...
    var routeRow = { column: mark.column, pool: pool };
    ...
    if (mark.kind === "guaranteed") {
      ...
      var guaranteedAction = guaranteedRouteAction(
        routeRow, guaranteed, mark);
      return routeChild(candidate, {
        ...
        cats: guaranteedAction.pulls.map(function (pull) { return pull.cat; }),
        actions: [guaranteedAction]
      }, order);
    }
    ...
    var avoidedR = false;
    if (!rolled.rerolled && !pool.platinum) {
      avoidedR = usableRouteRows(snapshot, false).some(function (other) {
        if (other.column === mark.column) return false;
        try {
          return !!engine.simulateRegular(other.pool, snapshot.seed,
            candidate.offset, candidate.lastRareId,
            snapshot.formIndex || 0).rerolled;
        } catch (_error) {
          return false;
        }
      });
    }
    var selectedAction = regularRouteAction(
      routeRow, rolled, mark.position, true, avoidedR);
    return routeChild(candidate, {
  ...
  function materializeRoute(candidate) {
    ...
    return steps.reduce(function (result, step) {
      ...
      result.actions = result.actions.concat(step.actions || []);
      return result;
    }, { auto: [], destinations: [], selections: [], cats: [], actions: [] });
  }
  ...
  function routeResourceUsage(snapshot, routePlan, marks) {
    var usage = { guaranteedPulls: 0, specialTickets: 0 };
    if (!routePlan || !routePlan.valid || !snapshot ||
        !Array.isArray(snapshot.rows)) return usage;
    var seenGuaranteed = Object.create(null);
    (Array.isArray(routePlan.selections) ? routePlan.selections : [])
      .forEach(function (step) {
        if (!step || step.kind !== "guaranteed") return;
        var key = step.column + ":" + step.position;
        if (seenGuaranteed[key]) return;
        seenGuaranteed[key] = true;
        usage.guaranteedPulls += 1;
      });

    var trackShape = { rows: snapshot.rows, count: snapshot.count };
    var actions = (Array.isArray(routePlan.auto) ? routePlan.auto : [])
      .concat(normalizeMarks(marks, trackShape));
    var seenSpecial = Object.create(null);
    actions.forEach(function (step) {
      if (!step) return;
      var row = snapshot.rows[step.column];
      var key = step.column + ":" + step.position;
      if (!row || !row.pool || !row.pool.platinum || seenSpecial[key]) return;
      seenSpecial[key] = true;
      usage.specialTickets += 1;
    });
    return usage;
  }

  function planProgressSummary(snapshot, routePlan, marks) {
    var selected = Array.isArray(marks) ? marks : [];
    var automatic = routePlan && Array.isArray(routePlan.auto) ?
      routePlan.auto : [];
    var usage = routeResourceUsage(snapshot, routePlan, selected);
    return "강조 좌표 " + selected.length + "개 · 자동 경로 " +
      automatic.length + "칸 · 확정뽑기 " + usage.guaranteedPulls +
      "회 · 특수티켓 " + usage.specialTickets + "회";
  }

  function planRowLabel(row, column) {
    var label = limitedText(row && (row.label || row.title ||
      row.customName || row.event), 180).trim();
    return label || "배너 " + (column + 1);
  }

  function routeTextSegment(text) {
    return { text: String(text || "") };
  }

  function routeCatSegment(cat, label, targetIds, selected) {
    var id = parseInt(cat && cat.id, 10);
    return {
      cat: cat || { id: id, name: "" },
      label: String(label || cat && cat.name || ""),
      target: targetIds.indexOf(id) !== -1,
      selected: selected === true
    };
  }

  function planRouteEntries(snapshot, routePlan, _marks, rawTargetIds) {
    if (!routePlan || !routePlan.valid || !snapshot ||
        !Array.isArray(snapshot.rows)) return [];
    var targetIds = normalizeTargetCatIds(rawTargetIds);
    var actions = Array.isArray(routePlan.actions) ? routePlan.actions : [];
    var entries = [];
    var run = null;
    ...
    function flushRun() {
      if (!run) return;
      entries.push({
        position: run.first === run.last ? run.first :
          run.first + "~" + run.last,
        segments: [routeTextSegment("레어티켓 " + run.count + "회")],
        next: run.next
      });
      run = null;
    }
    ...
    actions.forEach(function (action) {
      ...
      if (action.type === "guaranteed") {
        flushRun();
        var guaranteedSegments = [routeTextSegment(
          planRowLabel(snapshot.rows[action.column], action.column) +
          " · 확뽑")];
        (action.pulls || []).forEach(function (pull) {
          if (!pull || !pull.cat) return;
          var isTarget = targetIds.indexOf(parseInt(pull.cat.id, 10)) !== -1;
          var isSelected = action.selected === true && pull.guaranteed === true;
          if (!isTarget && !isSelected) return;
          guaranteedSegments.push(routeTextSegment(" · "));
          guaranteedSegments.push(routeCatSegment(
            pull.cat, pull.cat.name + " @ " + pull.position + " 획득",
            targetIds, isSelected));
        });
        entries.push({
          position: action.position + "~" + action.guaranteedLabel,
          segments: guaranteedSegments,
          next: action.next
        });
        return;
      }

      var cat = action.cat || null;
      var isTarget = !!(cat &&
        targetIds.indexOf(parseInt(cat.id, 10)) !== -1);
      var important = action.kind === "reroll" || action.avoidedR ||
        action.ticketKind || action.selected || isTarget;
      if (!important) {
        if (!run) {
          run = {
      ...
      flushRun();
      var banner = planRowLabel(snapshot.rows[action.column], action.column);
      var segments = [];
      if (action.kind === "reroll") {
        segments.push(routeTextSegment(banner + " · R 열변경 (" +
          (action.originalName || "중복 레어") + " → "));
        segments.push(routeCatSegment(
          cat, cat && cat.name, targetIds, action.selected));
        segments.push(routeTextSegment(")"));
      } else {
        var resource = action.ticketKind === "legend" ? "레전드 티켓" :
          (action.ticketKind === "platinum" ? "플래티넘 티켓" : "");
        var prefix = banner + " · ";
        if (action.avoidedR) prefix += "R 방어를 위해 이 배너에서 뽑기 · ";
        if (resource) prefix += resource + " · ";
        segments.push(routeTextSegment(prefix));
        segments.push(routeCatSegment(
          cat, cat && cat.name, targetIds, action.selected));
      }
      entries.push({
        position: action.resultLabel || action.position,
        segments: segments,
        next: action.next
      });
    });
  ...
  function planRouteEntryText(entry) {
    var body = (entry.segments || []).map(function (segment) {
      if (!segment.cat) return segment.text || "";
      return (segment.label || segment.cat.name || "") +
        (segment.target ? " [목표]" : "") +
        (segment.selected ? " [선택]" : "");
    }).join("");
    return entry.position + " · " + body +
      (entry.next ? " → " + entry.next : "");
  }

  function planLogEntries(snapshot, routePlan, marks, targetIds) {
    return planRouteEntries(snapshot, routePlan, marks, targetIds)
      .map(planRouteEntryText);
  }
  ...
  function buildRoutePlan(engine, snapshot, marks, options) {
    ...
    for (var markIndex = 0; markIndex < orderedMarks.length; markIndex += 1) {
      ...
      if (!candidates.length) {
        ...
        result.actions = partial.actions;
        result.invalid = mark;
    ...
    result.actions = materialized.actions;
    result.explored = explored.count;
  ...
  return {
    ...
    missingTargetCats: missingTargetCats,
    normalizeTargetCatIds: normalizeTargetCatIds,
    emptyRoutePlan: emptyRoutePlan,
    specialTicketRouteHint: specialTicketRouteHint,
    routeResourceUsage: routeResourceUsage,
    planProgressSummary: planProgressSummary,
    planRouteEntries: planRouteEntries,
    planRouteEntryText: planRouteEntryText,
    planLogEntries: planLogEntries,
