(function (root, factory) {
  ...
  function renderPlanLog() {
    var snapshot = trackApp.getFindSnapshot && trackApp.getFindSnapshot();
    var findCompleted = !!(root.MultiFindApp &&
      typeof root.MultiFindApp.getDestination === "function" &&
      root.MultiFindApp.getDestination());
    var entries = helpers.planLogEntries(
      snapshot, routePlan, marks, findCompleted);
  ...
  function planLogEntries(snapshot, routePlan, marks, findCompleted) {
    var entries = findCompleted ? ["Find route · 완료"] : [];
    if (!routePlan || !routePlan.valid || !snapshot ||
        !Array.isArray(snapshot.rows)) return entries;
    var trackShape = { rows: snapshot.rows, count: snapshot.count };
    var destinations = Array.isArray(routePlan.destinations) ?
      routePlan.destinations : [];
    var actions = (Array.isArray(routePlan.auto) ? routePlan.auto : [])
      .concat(normalizeMarks(marks, trackShape));
    var records = [];

    function destinationAfter(action, kind) {
      var sourceOffset = positionOffset(action.position);
      return destinations.filter(function (destination) {
        return destination && destination.kind === kind &&
          destination.column === action.column &&
          positionOffset(destination.position) > sourceOffset;
      }).sort(function (left, right) {
        return positionOffset(left.position) - positionOffset(right.position);
      })[0] || null;
    }

    (Array.isArray(routePlan.selections) ? routePlan.selections : [])
      .filter(function (selection) {
        return selection && selection.kind === "guaranteed";
      }).forEach(function (selection) {
        var destination = destinationAfter(selection, "guaranteed");
        records.push({
          position: selection.position,
          order: 1,
          text: selection.position + " · " +
            planRowLabel(snapshot.rows[selection.column], selection.column) +
            " · 확정뽑기" + (destination ? " → " + destination.position : "")
        });
      });

    actions.filter(function (action) {
      return action && action.kind === "reroll";
    }).forEach(function (action) {
      var destination = destinationAfter(action, "reroll");
      records.push({
        position: action.position,
        order: 2,
        text: action.position + " · " +
          planRowLabel(snapshot.rows[action.column], action.column) +
          " · R 열변경" + (destination ? " → " + destination.position : "")
      });
    });
    ...
    actions.forEach(function (action) {
      if (!action) return;
      var row = snapshot.rows[action.column];
      var key = action.column + ":" + action.position;
      if (!row || !row.pool || !row.pool.platinum || seenSpecial[key]) return;
      seenSpecial[key] = true;
      var ticket = String(row.pool.platinum).toLowerCase() === "legend" ?
        "레전드 티켓" : "플래티넘 티켓";
      records.push({
        position: action.position,
        order: 3,
        text: action.position + " · " + planRowLabel(row, action.column) +
          " · " + ticket + " 사용"
      });
    });
    ...
    records.sort(function (left, right) {
      return positionOffset(left.position) - positionOffset(right.position) ||
        left.order - right.order || left.text.localeCompare(right.text);
    }).forEach(function (record) { entries.push(record.text); });
    return entries;
  }
