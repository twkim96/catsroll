(function (root, factory) {
    ...
    return [
      ...
      '<div class="multi-plan-cats-tags">', entries.map(function (entry) {
        var rarityClass = entry.rarity === 5 ? "is-legend" :
          (entry.rarity === 4 ? "is-uber" : "is-supa");
  ...
  function renderSelectedCats() {
    ...
    selectedCatEntries = helpers.catSummaryEntries(cats, allowedSuperRareIds);
    var summary = helpers.summarizeCats(cats, allowedSuperRareIds);
    els.selectedCats.textContent = summary || "선택 없음";
    els.selectedCats.title = summary ||
      "선택된 비정규 슈퍼레어·울슈레·레전드레어가 없습니다.";
    ...
    els.selectedCats.setAttribute("aria-label", summary ?
      "선택 비정규 슈퍼레어·울슈레·레전드레어 전체 보기: " + summary :
      "선택된 비정규 슈퍼레어·울슈레·레전드레어가 없습니다.");
  ...
  function catSummaryEntries(cats, allowedSuperRareIds) {
    var entries = [];
    ...
    (Array.isArray(cats) ? cats : []).forEach(function (cat) {
      ...
      if (!name || ([4, 5].indexOf(rarity) === -1 &&
          !(rarity === 3 && allowedSuperRare[id]))) return;
  ...
  function summarizeCats(cats, allowedSuperRareIds) {
    return catSummaryEntries(cats, allowedSuperRareIds).map(function (entry) {
  ...
  function missingTargetCats(targets, acquiredEntries) {
    ...
    return (Array.isArray(targets) ? targets : []).map(function (target) {
      ...
      if (!(id > 0) || !name || [3, 4, 5].indexOf(rarity) === -1 ||
