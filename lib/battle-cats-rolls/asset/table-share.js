(function () {
  ...
  function renderedRowLimit(target) {
    if (isMultiPage()) {
      if (window.MultiTrackApp &&
          typeof window.MultiTrackApp.getRowCount === "function") {
        return window.MultiTrackApp.getRowCount();
      }
  ...
  function stagedTarget(target, limit) {
    ...
    if (isMultiPage()) {
      var tables = target.querySelectorAll(".multi-track-table");
      var gap = parseFloat(window.getComputedStyle(target).columnGap) || 0;
      var fullTableWidth = Array.prototype.reduce.call(tables, function (sum, table) {
        return sum + Math.ceil(table.scrollWidth);
      }, 0) + Math.max(0, tables.length - 1) * gap;
      width = Math.max(width, fullTableWidth, 1200);
    }
    ...
    var populated = isMultiPage() && window.MultiTrackApp &&
      typeof window.MultiTrackApp.populateCaptureClone === "function" &&
      window.MultiTrackApp.populateCaptureClone(copy, limit);
    if (!populated) trimRows(copy, limit);
    copy.style.width = width + "px";
    copy.style.maxWidth = "none";
    copy.style.margin = "0";
    copy.style.background = "#fff";
    if (copy.classList.contains("is-scrollable")) {
      copy.style.display = "grid";
      copy.style.gridTemplateColumns = "repeat(2, minmax(0, 1fr))";
    ...
    copy.querySelectorAll(".multi-track-scroll-controls").forEach(function (controls) {
      controls.remove();
    });
    stage.appendChild(copy);
    var host = target.closest(".multi-track-app") || document.body;
    host.appendChild(stage);
    if (populated && window.MultiTrackApp &&
        typeof window.MultiTrackApp.syncCaptureRows === "function") {
      window.MultiTrackApp.syncCaptureRows(copy);
    }
