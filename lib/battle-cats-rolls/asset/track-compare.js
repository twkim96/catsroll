(function () {
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      "td.score.track-compare-cell{position:relative;padding:0;vertical-align:top}",
      ".track-compare-boxes{display:flex;flex-direction:column;",
      "gap:0;margin:0;width:100%;position:absolute;top:0;left:0;right:0;z-index:1}",
      ".track-compare-box{box-sizing:border-box;height:2.35em;width:100%;",
      "max-width:none;padding:0;border:0;border-bottom:1px solid #b3b3b3;",
      "border-radius:0;background:transparent;color:#123;line-height:2.35em;font-size:.56em;",
      "appearance:none;-webkit-appearance:none;text-align:center;overflow:hidden;",
      "white-space:nowrap;text-overflow:clip;cursor:pointer}",
      ".track-compare-box::-webkit-scrollbar{display:none}",
      ".track-compare-box{scrollbar-width:none;-ms-overflow-style:none}",
      ...
      ".track-compare-slot-title{font-weight:bold}",
      ".track-compare-slot-label-row{display:grid;align-items:start;",
      "grid-template-columns:4.5rem 1fr 5.5rem;gap:.45rem;margin:.8rem 0 -.25rem}",
      ".track-compare-slot-label{grid-column:2 / 4;font-size:.72em;",
      "font-weight:normal;color:#666;line-height:1.2;max-height:2.4em;overflow:hidden}",
      ".track-compare-modal-actions{text-align:right;margin-top:.8rem}",
      ".track-compare-modal button{margin-left:.35rem}",
      "@media (min-width:761px){.track-compare-boxes.two{display:grid;",
      "grid-template-columns:minmax(0,1fr) minmax(0,1fr)}}",
      "@media (max-width:760px){.track-compare-box{font-size:.52em;",
      "height:2.5em;line-height:1.2;white-space:normal;overflow-wrap:anywhere;",
      "display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2}",
      ".track-compare-slot,.track-compare-slot-label-row{grid-template-columns:1fr}",
      ".track-compare-slot-label{grid-column:1}.track-compare-slot-title{margin-top:1rem}}"
    ].join("");
  ...
  function renderSlot(list, index, selected) {
    ...
    labelRow.className = "track-compare-slot-label-row";
    var title = document.createElement("div");
    title.className = "track-compare-slot-title";
    title.textContent = "확장 " + (index + 1);
    var titleLabel = document.createElement("div");
    titleLabel.className = "track-compare-slot-label";
    ...
    updateSlotTitleLabel(event, titleLabel);
  ...
  function updateSlotTitleLabel(select, label) {
    var option = select && select.options[select.selectedIndex];
    var text = option && option.value ?
  ...
  function loadButton(cell, button) {
    ...
    button.textContent = "...";
    return fetch(resultUrl(cell, entry), {headers: {"Accept": "application/json"}}).
      then(function (response) {
        if (!response.ok) {
  ...
  function loadCell(cell) {
    return Promise.all(Array.prototype.map.call(
      cell.querySelectorAll(".track-compare-box"), function (button) {
        return loadButton(cell, button);
      }));
  }

  function highValueCell(cell) {
    return cell.classList.contains("uber_fest") ||
      cell.classList.contains("uber") ||
      cell.classList.contains("legend") ||
      cell.classList.contains("exclusive");
  }

  function enqueueAutoCell(cell) {
    if (!cell || cell.trackCompareQueued || cell.trackCompareAutoLoaded) {
      return;
    }
    cell.trackCompareQueued = true;
    autoQueue.push(cell);
    pumpAutoQueue();
  }

  function nearViewport(cell) {
  ...
  function pumpAutoQueue() {
    ...
    var cell = autoQueue.shift();
    if (!cell) {
      return;
    }
    if (!nearViewport(cell)) {
      cell.trackCompareQueued = false;
      observeAutoCell(cell);
      window.setTimeout(pumpAutoQueue, 0);
      return;
    }

    autoLoading = true;
    loadCell(cell).then(function () {
      cell.trackCompareAutoLoaded = true;
    }).finally(function () {
      cell.trackCompareQueued = false;
      autoLoading = false;
      window.setTimeout(pumpAutoQueue, 120);
    });
  }

  function resetAutoObserver() {
    autoQueue = [];
    autoLoading = false;
    if (autoObserver) {
  ...
  function observeAutoCell(cell) {
    if (!highValueCell(cell) || !("IntersectionObserver" in window)) {
      return;
    }
    if (!autoObserver) {
      autoObserver = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            ...
            enqueueAutoCell(entry.target);
          }
        });
      }, {rootMargin: "240px 0px"});
    }
    autoObserver.observe(cell);
  }
  ...
  function renderBoxes() {
    ...
    var settings = readSettings();
    var cells = document.querySelectorAll("td.score[data-expand-slot-seed]");
    ...
    Array.prototype.forEach.call(cells, function (cell) {
      ...
      if (!settings.events.length) {
        cell.classList.remove("track-compare-cell");
        return;
      }
      cell.classList.add("track-compare-cell");
      cell.trackCompareAutoLoaded = false;
      cell.trackCompareQueued = false;

      var wrap = document.createElement("div");
      wrap.className = "track-compare-boxes" +
        (settings.events.length > 1 ? " two" : "");
