(function () {
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ".track-compare-open{margin-left:.35rem;padding:.15rem .45rem;",
      ...
      ".track-compare-modal h3{font-size:1rem;margin:.85rem 0 .35rem}",
      ".track-compare-row{display:grid;grid-template-columns:auto 1fr 5.5rem;",
      "gap:.45rem;align-items:center;margin:.3rem 0}",
      ".track-compare-row span{overflow:hidden;text-overflow:ellipsis;",
      "white-space:nowrap}",
      ".track-compare-row input[type=number]{width:5rem}",
      ...
      ".track-compare-row{grid-template-columns:auto 1fr}.track-compare-row ",
      "input[type=number]{grid-column:2;width:5rem}}"
    ].join("");
  ...
  function openModal() {
    ...
    clear.addEventListener("click", function () {
      Array.prototype.forEach.call(
        list.querySelectorAll("input[type=checkbox]"), function (input) {
          input.checked = false;
        });
  ...
  function renderModalList(list, lists) {
    ...
    var selected = settings.events.reduce(function (result, entry) {
      result[entry.lang + "|" + entry.event] = entry;
      return result;
    }, {});
    ...
    lists.forEach(function (data) {
      ...
      heading.textContent = data.lang.toUpperCase();
      list.appendChild(heading);

      data.events.forEach(function (event) {
        var baseId = data.lang + "|" + event.event;
        var selectedEntry = selected[baseId];
        ...
        row.className = "track-compare-row";

        var checkbox = document.createElement("input");
        checkbox.type = "checkbox";
        checkbox.checked = !!selectedEntry;
        checkbox.setAttribute("data-lang", data.lang);
        checkbox.setAttribute("data-event", event.event);
        checkbox.setAttribute("data-label", event.label);
        ...
        ubers.addEventListener("click", function (event) {
          event.stopPropagation();
        });
  ...
  function updateSelection(list, keepCache) {
    ...
    if (checked.length > maxEvents) {
      checked.slice(maxEvents).forEach(function (input) {
        input.checked = false;
      });
      checked = checked.slice(0, maxEvents);
    }

    Array.prototype.forEach.call(
      list.querySelectorAll("input[type=checkbox]"), function (input) {
        input.disabled = !input.checked && checked.length >= maxEvents;
      });

    var next = checked.map(function (input) {
      var row = input.parentNode || input.parentElement;
      var ubers = row && row.querySelector("input[type=number]");
      return {
        lang: input.getAttribute("data-lang"),
        event: input.getAttribute("data-event"),
