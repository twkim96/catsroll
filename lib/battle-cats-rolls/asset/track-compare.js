(function () {
  ...
  function closestCell(node) {
    while (node) {
      if (node.tagName === "TD" && node.classList.contains("cat")) {
        return node;
      }
  ...
  function alignBoxes(cell) {
    if (!cell || cell.trackCompareAligning) {
      return;
    }
    cell.trackCompareAligning = true;
    window.requestAnimationFrame(function () {
      var wrap = cell.querySelector(".track-compare-boxes");
      cell.trackCompareAligning = false;
      if (!wrap) {
        return;
      }

      wrap.style.setProperty("--track-compare-offset", "0px");
      var cellTop = cell.getBoundingClientRect().top;
      var wrapTop = wrap.getBoundingClientRect().top;
      var offset = Math.round(cellTop + 1 - wrapTop);
      wrap.style.setProperty(
        "--track-compare-offset", offset < 0 ? offset + "px" : "0px");
    });
  }
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      ".track-compare-boxes{display:flex;flex-direction:column;gap:.38em;",
      "position:relative;z-index:1;margin:0 0 .45em;width:100%;",
      "transform:translateY(var(--track-compare-offset,0px))}",
      ".track-compare-box{box-sizing:border-box;min-height:.72em;width:100%;",
      ...
      ".track-compare-box:not(:empty){min-height:2.35em;padding-bottom:.12em;",
